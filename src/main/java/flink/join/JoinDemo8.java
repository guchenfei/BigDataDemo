...
public class JoinDemo8 {
    public static void main(String[] args) throws Exception {
        ...
        String kafkaIPs = "localhost:9092";
        Properties props = new Properties();
        props.setProperty("bootstrap.servers", kafkaIPs);
        props.setProperty("group.id", "group.cyb.2");

        //读取用户信息Kafka
        FlinkKafkaConsumer<UserInfo> userConsumer = new FlinkKafkaConsumer<UserInfo>("user2", new UserInfoSchema(), props);
        ...
        //读取城市维度信息Kafka
        FlinkKafkaConsumer<CityInfo> cityConsumer = new FlinkKafkaConsumer<CityInfo>("city2", new CityInfoSchema(), props);
